...
def obtener_resultado(nombre):
    ...
    for i in range(pila_temas.top + 1):
        ...
        for j in range(tema.preguntas.top + 1):
            ...
            p= Pila(pregunta.encuestados.size)
            for k in range(pregunta.encuestados.top+1):
                p.push(accederPosicion(pregunta.encuestados,k).id)
                
            
            print(f"[{pregunta.promedio_opinion}] {pregunta.nombre} ({pregunta.encuestados}))")
