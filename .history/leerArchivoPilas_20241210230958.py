...
""" def obtener_encuestado_por_preguntas(encuestados,temas):
    lista_temas = []
    lista_tema_pregunta = []
    lista_encuestado_preg = []
    i = 1
    idx = 1
    for tema in temas:
        for pregunta in tema:
            for encuestado in pregunta:
                lista_encuestado_preg.append(encuestados[encuestado-1])
                idx+=1
            lista_tema_pregunta.append(lista_encuestado_preg)
            i += 1
            lista_encuestado_preg = []
            
            
        lista_temas.append(Pregunta(f"Pregunta {i}.{idx + 1}",lista_tema_pregunta))
        i = 1
        idx= 1
...
def obtener_encuestado_por_preguntas(encuestados, temas):
    ...
    for i, tema in enumerate(temas, start=1):  # Iterar sobre temas con índice 1, 2, ...
        lista_tema_pregunta = []
        
        for idx, pregunta in enumerate(tema, start=1):  # Iterar sobre preguntas con índice 1, 2, ...
            lista_encuestado_preg = []
            
            for encuestado in pregunta:  # Recorrer los encuestados por pregunta
                lista_encuestado_preg.append(encuestados[encuestado - 1])
            
            # Crear una nueva pregunta con el índice correcto
            lista_tema_pregunta.append(
                Pregunta(f"Pregunta {i}.{idx}", lista_encuestado_preg)
            )
        
        # Agregar las preguntas del tema actual al resultado final
        lista_temas.extend(lista_tema_pregunta)
    
    return lista_temas

    
""" 
def crear_objeto_preguntas(temas):
...
def obtener_pilas(nombre):
    ...
    temas_obj = obtener_encuestado_por_preguntas(encuestados,lista_datos[1])
...
print(pila_temas.pila[0].pila[0])
        
        
        
print(promedio(pila_temas.pila[0].pila[0],pila_encuestados,'experticia'))
