...
def promedio(preguntas,tipo):
    acum = 0
    for pregunta in range(preguntas.preguntas.top+1) :
        for id in range ((accederPosicion(preguntas.pregunta,pregunta).encuestados.top)+1):
            if tipo == 'opinion':
                acum += accederPosicion(accederPosicion(preguntas.pregunta,pregunta).encuestados,id).opinion
            else:
                acum += accederPosicion(accederPosicion(preguntas.pregunta,pregunta).encuestados,id).experticia 
        if tipo == 'opinion':  
            accederPosicion(preguntas.pregunta,pregunta).promedio_opinion = acum/((accederPosicion(preguntas.pregunta,pregunta).encuestados.top)+1)
        else:
            accederPosicion(preguntas.pregunta,pregunta).promedio_experticia = acum/((accederPosicion(preguntas.pregunta,pregunta).encuestados.top)+1)
