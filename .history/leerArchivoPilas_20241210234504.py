...
print(accederPosicion(accederPosicion(pila_temas,0),0).promedio_opinion)
...
def promedio(preguntas,tipo):
    ...
    for pregunta in range(preguntas.top) :
        for encuestado in accederPosicion(preguntas,pregunta).encuestados:
            if tipo == 'opinion':
                acum += encuestado.opinion
    return acum
            

promedio(accederPosicion(pila_temas,0),'opinion')
